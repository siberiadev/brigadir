...
/**
 * Feature 028 (T021) — the per-profile gate and live type-capacity re-apply
 * work on `run.deepseek_api` exactly as on the other run queues: the
 * DeepseekRunProcessor worker's concurrency converges to the sum of ENABLED
 * deepseek_api profiles' max_parallel_runs (and only those — the claude_cli
 * worker keeps its own budget), and a limit-1 deepseek profile never runs two
 * jobs at once while gating burns no attempt. Mirrors kimi-gate.spec.ts.
 */
describe('deepseek per-profile gate + type capacity (feature 028, T021)', () => {
  let db: DbHarness;
  let redis: RedisHarness;
  let worker: TestingModule;
  let trigger: RunTriggerService;
  let env: ClaudeCliTestEnv;
  let deepseekProc: DeepseekRunProcessor;
  ...
  beforeAll(async () => {
    ...
    await worker.init();
    deepseekProc = worker.get(DeepseekRunProcessor);
    claudeProc = worker.get(ClaudeCliRunProcessor);
    await deepseekProc.worker.waitUntilReady();
  ...
  async function seedDeepseekProfile(name: string, maxParallelRuns: number) {
    const [row] = await db.db
      ...
      .values({
        ...
        name,
        maxParallelRuns,
        config: baseExecutorConfig(env, { model: 'deepseek-v4-flash' }),
        secrets: sealExecutorSecrets({ api_key: 'sk-deepseek-gate' }),
      })
  ...
  async function seedAgentWithTicket(executorId: string, name: string, ticketKey: string) {
    ...
    const [ticket] = await db.db
      ...
      .values({ workspaceId, jiraKey: ticketKey, jiraId: ticketKey, summary: 'deepseek gate' })
  ...
  it('deepseek type capacity converges to the enabled-profiles sum; limit-1 profile never doubles up; no attempt burned', async () => {
    const profileA = await seedDeepseekProfile('deepseek-gate-a-limit1', 1);
    await seedDeepseekProfile('deepseek-gate-b-limit2', 2);

    const a1 = await seedAgentWithTicket(profileA, 'deepseek-a1', 'DGATE-1');
    const a2 = await seedAgentWithTicket(profileA, 'deepseek-a2', 'DGATE-2');

    // Re-apply tick lifts the deepseek worker to the type capacity (1+2=3)…
    const capDeadline = Date.now() + 10_000;
    while (deepseekProc.worker.concurrency !== 3) {
      if (Date.now() > capDeadline) {
        throw new Error(
          `deepseek type capacity never reached 3 (at ${deepseekProc.worker.concurrency})`,
        );
      }
      await new Promise((r) => setTimeout(r, 50));
    }
    // …while the claude_cli worker's budget is untouched by deepseek profiles
    ...
    // transactional — same discipline as the mock/kimi gate suites).
    {
      const deadline = Date.now() + 15_000;
      for (;;) {
        const [r] = await statusesOf([first.runId]);
        if (r?.status === 'running') break;
        if (Date.now() > deadline) throw new Error('first deepseek run never started');
    ...
    for (;;) {
      ...
      if (Date.now() > deadline) throw new Error(`deepseek runs stuck: ${JSON.stringify(rows)}`);
