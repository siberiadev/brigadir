...
- **2026-07-21 — Compliance fix: FR-038 enforced on the dependency-release path.** `DependencyReleaseService` (feature 022-sprint-sequencing), which re-fetches and re-triggers tickets sitting in a trigger status, was found to omit `scope_jql`/board-sprint scope from its Jira re-fetch — a violation of this file's existing FR-038 text ("a ticket that does not satisfy `scope_jql` is never ingested and never triggers an agent — even while it sits in an agent's trigger status"), reproduced live on a scrum board running two simultaneously active sprints where `scope_jql` pinned one. FR-038's own wording required no change; the fix and its cross-reference are recorded in `specs/022-sprint-sequencing/spec.md`'s Revision History (FR-002 amendment) and `specs/022-sprint-sequencing/research.md` (R1 update).
