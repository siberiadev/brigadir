...
2. **Given** a setup run whose submitted proposal fails validation on any single item, **When** the completion is processed, **Then** no agent is created at all and the completion is rejected back to the agent with the specific validation errors (the agent may correct and re-submit within the run); **and Given** the run ends without an accepted proposal, **Then** it is failed with the last validation errors recorded on its report/timeline and a human task describing the failure is created.
...
- **FR-017**: An invalid proposal MUST be rejected back to the agent as a completion validation error carrying the specific issues, giving the agent the chance to correct and re-submit within the same run (no agents are created by a rejected proposal). A setup run that ends without an accepted proposal MUST be failed with the last validation errors recorded on the run's report/timeline and surfaced in a human task describing the failure. *(Amended at planning, 2026-07-16 — see research.md D9: the repair loop replaces insta-fail; the failure guarantee is unchanged.)*
