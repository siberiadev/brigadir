...
- **FR-026** *(answer-triage delta)*: A routed decision from an `answer-triage` run MUST be exempt from the exhausted-budget override (the human answer grants one extra rework cycle, exactly once — the decision-time check keys off the deciding run's trigger source, and the exemption applies only while `cycle_count < rework_max + 1`: the grant is a hard cap, not a blanket bypass). The resulting rework run keeps `source='rework'` and counts in the budget; invalid-target overrides still apply; the automatic fail-triage cap (FR-005/006) is unchanged. The deciding human task's id MUST be forwarded into the rework trigger for traceability.
