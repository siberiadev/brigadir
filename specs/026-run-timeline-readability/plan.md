...
- **V. Secret Isolation & Output Scrubbing** — ATTENTION → addressed in design. This feature persists **more** agent-authored text into `run_events` (full progress/assistant text, structured tool inputs). Parser-side `tool_call` events are today written without the report scrubber. Decision (research R6, hardened per analysis finding C1): the sanitizer applies the existing `@brigadir/scrubber` `scrub()` to **every retained string field at any depth** — including strings nested inside a `complete_task` report (`checks[].reason`, `artifacts.*`), not only top-level fields — at the executor persist boundary, so raising fidelity does not widen secret exposure and no nested agent string reaches the DB unscrubbed. Human-text fields are scrubbed too (matching the callback path, which already scrubs `message`/`title`/`details`/`summary`).
