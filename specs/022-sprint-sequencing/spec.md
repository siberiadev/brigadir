...
- **Blocker lives outside the observed board scope** (another project or filtered out): its completion may never be observed; the dependent shows as waiting indefinitely and falls under Story 4 diagnosability rather than silently vanishing.
...
- **FR-001**: When a ticket in an agent's trigger status is found blocked, the system MUST record that state — including the set of blocking ticket keys — rather than discarding the event; the record MUST be kept current as blockers complete.
- **FR-002**: When a ticket transitions into the completion status category, the system MUST re-evaluate every ticket it was blocking; each dependent that is currently in an enabled agent's trigger status and clear on ALL of its blockers MUST be started automatically, with no human action.
- **FR-003**: The release re-check MUST fire for both blocker-completion paths: a completion observed from the board (human or external change) and a completion performed by the system itself after a successful run. Both paths MUST produce the same outcome for dependents.
...
- **FR-010**: The system MUST surface, as an operator-visible warning distinguishable from ordinary waiting, a dependent whose blocker sits in a state that cannot reach the completion category through normal work (dead-end wait); resolution remains a human action, after which the normal release path applies.
...
- **Release event**: the re-evaluation caused by a blocker reaching the completion category; produces zero or more ordered ticket starts.
...
- **Visibility derives from already-observed board data** (dependency links the system already collects); no new operator input is required to see waiting states.
- **Blockers outside the observed scope** cannot have their completion observed; such waits are legitimate long waits surfaced through diagnosability (Story 4), not errors.
- **Reconciliation cadence bounds release latency**: "automatic" means within the system's normal observation cycle, not instantaneous.

## Out of Scope

- **Concurrent-run cap per epic/chain** — a limit so a wave of N simultaneously released tickets does not pile into the same repository in parallel. Explicitly deferred pending the clarify phase (the brief flags it "raise during clarify, may be deferred"); if adopted later it is a separate decision and does not change the requirements above.
