...
export function loadAgentsConfig(path: string = resolveAgentsConfigPath()): AgentsConfig {
  ...
  try {
    ...
    throw new AgentsConfigError(
      `agents config not found or unreadable at ${abs}: ${(err as Error).message}`,
    );
  }
...
/** DI provider that loads+validates the config eagerly at module init (fail-fast). */
export const agentsConfigProvider: Provider = {
  provide: AGENTS_CONFIG,
  useFactory: (): AgentsConfig => loadAgentsConfig(),
};
