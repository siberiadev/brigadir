...
describe('PlatformSettings — sub-nav + executors panel', () => {
  ...
  it('General is theme-only since feature 015 — the instruction fields moved to Brigadir agent', async () => {
    setDashboardToken('test-token');
    server.use(http.get('/api/human-tasks/count', () => HttpResponse.json({ open: 0 })));
    const wrapper = mountWithProviders(App, { routes, initialPath: '/settings/general' });
    await wrapper.vm.$router.isReady();
    ...
    expect(wrapper.find('[data-test="theme-mode"]').exists()).toBe(true);
    expect(wrapper.find('[data-test="default-orchestrator-instruction"]').exists()).toBe(false);
    expect(wrapper.find('[data-test="workspace-setup-instruction"]').exists()).toBe(false);
    // The relocated fields live under the third sub-nav item.
    expect(wrapper.find('[data-test="settings-nav-brigadir-agent"]').exists()).toBe(true);
  });
