...
-- Поллер пишет его на КАЖДОМ проходе (`[]`, если входящих связей нет), пайплайн
-- и релиз-пасс — тоже; релиз больше НЕ обнуляет колонку. Причина: prepare
-- зависимого прогона читает её, чтобы стартовать от веток блокеров, — а это
-- происходит уже ПОСЛЕ того, как тикет перестал ждать. Состояние ожидания живёт
-- только в `blocked_state`; все читатели ("кто ждёт?") фильтруют по нему.
-- Миграции/бэкфилла нет: NULL-строки самозалечиваются на следующем наблюдении.
...
-- третий рубеж идемпотентности: один активный прогон на ТИКЕТ
-- (SXF-1174 Problem 6, миграция 0011; ранее — на (ticket, agent): два агента
-- не должны работать над одним тикетом одновременно)
CREATE UNIQUE INDEX runs_one_active ON runs (ticket_id)
...
**Энфорсмент таймаутов на исполнителе (feature 034).** Промис executor'а резолвится по `close` процесса; если после kill'а (`terminate()`: SIGTERM → grace → SIGKILL группы + pgrep-добивание потомков) `close` не приходит за `settleGraceMs` (кламп [100, 60000], дефолт 5 с — сбежавший потомок с унаследованным stdio иначе вешает прогон навсегда), прогон принудительно резолвится из уже распознанного исхода (прецедентность cancelled > rate_limited > timeout) с run_events `type:'log'`, `payload.source:'settlement-timeout'`; фолбэк вооружают ОБА kill-пути — abort и rate-limit. `timeout_ms_override` из triggerEvent клампится 24 часами (`sanitizeTimeoutMs` — untrusted вход через `.passthrough()`). Фаза подготовки (clone/fetch/worktree) абортируема: границы фаз проверяют AbortSignal, сетевые git-операции получают его напрямую, и каждый git-вызов worktree-слоя несёт hang-breaker `WORKTREE_GIT_TIMEOUT_MS` (10 мин); abort в prepare даёт исход `cancelled`/`timeout`, не `crashed` (attempt не сжигается на отмене).

**Bootstrap воркктри + dirty-guard (feature 035, token-spend §4/§5).** Фаза подготовки после `prepareAll`: для каждого репо с `settings.repositories[].bootstrap_command` (несекретная строка, ≤500) executor запускает команду в свежем воркктри ДО спауна агента — `spawn('/bin/sh', ['-c', cmd])` detached с group-kill (не `execFile`: его timeout убивает только sh-лидера), hang-breaker `bootstrapTimeoutMs` (кламп [10 с, 30 мин], дефолт 10 мин), env = та же сборка, что у агентского чайлда (allowlist-floor + operator env, БЕЗ auth/provider-ключей) плюс `npm_config_cache=<pmCacheRoot>/npm` (общий кэш, дефолт `~/.brigadir/pm-cache` — переживает прогоны, т.к. воркктри одноразовые, а кэш нет). Исход пишется в run_events `type:'log'`, `payload.source:'bootstrap'` (command/exit_code/duration_ms/хвосты stdout/stderr — всё через run-scoped скраббер); не-нулевой exit или таймаут ФЕЙЛИТ прогон с диагностикой (retry-механика покрывает сетевые флаки, `keepFailedWorktrees` сохраняет место преступления). Затем — БЕЗУСЛОВНО для каждого репо (и без bootstrap) — dirty-guard: `git status --porcelain`; изменённые tracked-файлы revert'ятся (`git checkout -- .`) с событием `kind:'dirty-after-prepare'` (списки reverted/untracked + честный `residual_modified` после повторного status — «стало чисто» не утверждается), untracked остаются на месте. Wrapper при выполненном bootstrap'е сообщает агенту «bootstrap already ran / dependencies are installed» (repoLine + правило в Verification-секции) — вместо наблюдавшихся 204 переустановок и проб `ls node_modules` на 142 прогона. Легаси yaml-путь репозиториев bootstrap не получает; исходный кейс dirty-guard'а (st3_agentic: два spec-файла, 25 оплаченных переоткрытий) разобран — некононичные для prettier коммиты + `npm run format`/`lint --fix` у агентов.
...
- **Watchdog (feature 034, Jira-free)**: первый шаг каждого reconcile-пасса — глобальный `WatchdogService.sweepAll()` ДО резолва Jira-клиентов и цикла по воркспейсам (битые креды или упавшая интроспекция борды одного воркспейса раньше молча выключали жатву — класс инцидента 17-часового Reviewer-прогона). Свипятся ВСЕ воркспейсы, включая disabled (зомби-`running` иначе вечно держит `runs_one_active` и блокирует тикет); для disabled — только DB-флип + timeline-событие, Jira-обработку доносит drift repair после включения. Правила: `running` дольше `timeout_minutes + WATCHDOG_GRACE_MINUTES` (дефолт 5 мин) → `timed_out`; `queued` дольше `WATCHDOG_QUEUED_SWEEP_MINUTES` (дефолт 720 мин, заведомо больше легитимных rate-limit-репарковок) при `started_at IS NULL` → `failed` (ничего не стартовало — `timed_out` врал бы). Битое env-значение → warn + дефолт (NaN в SQL-интервале молча отключал бы sweep). Каждая жатва пишет run_events `type:'log'`, `payload.source:'watchdog'`. `awaiting_human` сознательно без таймаута; mock-путь (`run.mock`) собственного таймера не имеет — его страхует только watchdog.
...
Для Claude-executor'ов — `Stop`-hook: если ни `complete_task`, ни blocking `request_human` не вызывались в этой сессии (проверка по маркер-файлу, который пишет MCP-сервер после любого из них), hook возвращает `{"decision": "block", "reason": "You must call mcp__brigadir__complete_task with your final report before finishing."}`. Плюс жёсткий пояс: процесс завершился без `complete` и не в `awaiting_human` → статус из fallback-каналов (`--json-schema` structured_output, если есть) → иначе `failed`. Два уточнения (SXF-1174 Problem 7): (1) для callback-wired прогонов executor НЕ парсит structured_output на выходе (`--json-schema` не передаётся — FR-011, один живой канал), поэтому в `runs.error` попадает честная диагностика «exited without a complete_task or request_human callback», а не мусорное «no schema-valid structured_output»; (2) перед fail-closed воркер проверяет open **blocking**-задачу у прогона — если она есть, run паркуется `awaiting_human` (guarded `WHERE status='running'`) вместо `failed`: висящий вопрос человека не выбрасывается, даже если park при создании задачи был пропущен.
...
**Наследование веток блокеров (feature 032).** Между «своей прошлой работой» и дефолтной веткой
появился третий уровень: работа тикетов из `tickets.blocked_by`. Порядок подготовки —
`ensureCaches` (его `fetch --prune` и делает пробы правдивыми) → резолв блокеров → `prepareAll`.
Правила:

- **Precedence — по РЕПОЗИТОРИЮ, слияния между уровнями нет** (`buildStartPlan`): своя работа
  выигрывает только тот репо, который она называет; остальные всё равно наследуются от блокеров.
  Ключи блокеров сортируются лексикографически — первый назвавший репо задаёт старт-ветку, каждый
  следующий, назвавший ТОТ ЖЕ репо, попадает в `mergeBranches`.
- **Diamond'ы система сливает сама**, последовательно и `--no-ff`, в уже detached-воркти
  (`git -c user.name=brigadir -c user.email=brigadir@local merge --no-ff`); идентичность коммитера
  передаётся флагами `-c`, глобальный git-config система не пишет. Конфликт ⇒ `merge --abort`,
  `BlockerMergeConflictError`, штатный all-or-nothing unwind, задача `[blocker_merge_conflict]`.
  Автоматически конфликт не разрешается: машинно выбранное слияние работы двух агентов — ровно та
  тихая неправота, которой этот код избегает. `startSha` резолвится ПОСЛЕ последнего слияния, поэтому
  база completion-gate (§5) включает слитую работу по построению и правило feature 024 не меняется.
- **Асимметрия пропавшей ветки (FR-008).** Ветку, названную СВОЕЙ прошлой работой, но отсутствующую
  на origin, по-прежнему роняем громко. Ветка БЛОКЕРА, которой нет: если категория статуса блокера
  `done` — это нормальный конец цепочки (PR влит, ветка удалена), старт с дефолтной тихо, событие
  `blocker_branch_merged`; если блокер ещё открыт — старт с дефолтной, событие
  `blocker_no_artifact` и задача `[blocker_branch_lost]`. Категорию берём ОДНИМ батч-запросом
  `key in (…)` к живой Jira на этапе prepare (кэш тикетов категорию не хранит, а Принцип I делает
  Jira источником правды по статусу); сбой запроса роняет прогон с диагностикой — молчаливого
  фолбэка нет.
- **Монтирование не расширяется никогда (FR-010):** набор репо — это база агента, суженная
  Components тикета. Работа блокера в несмонтированном репо даёт событие
  `blocker_artifacts_unmounted` и задачу `[blocker_repo_unmounted]`, а не внезапный лишний клон.
- Новые `decision` в событии `start-ref`: `inherited_from_blocker`, `merged_blockers`,
  `blocker_branch_merged`, `blocker_no_artifact`, `blocker_artifacts_unmounted` (+ поля `blockers`,
  `mergedBranches`). Три задачи — run-less, `kind='blocker'`, дедуп по ТОЧНОМУ заголовку с машинным
  префиксом (`[blocker_branch_lost] …`): `HumanTaskKind` — версионируемый контракт агента, и
