...
## Iteration 50 — Bootstrap воркктри + dirty-guard (feature 035, token-spend §4/§5, 2026-07-30)

Два хвоста token-spend-анализа 2026-07-28, закрытые одним местом — фазой подготовки воркктри: агент должен находить готовое окружение, а не оплачивать его тёрнами. §5: 204 × `npm ci` на 142 прогона + защитные пробы `ls node_modules`. §4: 25 × `git checkout --` двух «таинственно грязных» spec-файлов st3_agentic (4 роли, 8 тикетов) — знание «это шум» не персистилось.

- **`bootstrap_command` per-repository** (`settings.repositories[]`, несекретная строка ≤500; admin-MCP `create_workspace` и dashboard create/PUT принимают, UI — поле в `RepositoryForm.vue`; легаси yaml-путь — осознанно НЕ поддержан). Новый модуль `libs/executors/src/claude-cli/bootstrap.ts`: `spawn('/bin/sh',['-c',cmd])` **detached + group-kill** (не `execFile` — его timeout убивает только sh-лидера, потомки npm сбегают: клин feature 034), rolling-хвосты stdout/stderr (не maxBuffer — болтливый `npm ci` его переполняет), hang-breaker `bootstrapTimeoutMs` (кламп [10 с, 30 мин], дефолт 10 мин) + AbortSignal прогона. Env — та же сборка, что у агентского чайлда (allowlist-floor + operator env 031, БЕЗ auth/provider-ключей) + `npm_config_cache=<pmCacheRoot>/npm` (общий кэш, дефолт `~/.brigadir/pm-cache`). Слот — во ВТОРОМ try (после записи `worktreePath`, до `writeMcpConfig`): фейл получает `cleanupWorkspace`+`keepFailedWorktrees`+abort-маппинг бесплатно (первый catch НЕ чистит воркктри — нашёл Plan-агент). Не-нулевой exit/таймаут ФЕЙЛИТ прогон с диагностикой (политика утверждена; ретраи покрывают флаки). Событие `source:'bootstrap'` (command/exit_code/duration_ms/хвосты, всё через run-scoped скраббер — оператор может вписать секрет в команду).
- **Dirty-guard — безусловный** (для каждого репо, и без bootstrap'а): `git status --porcelain` → revert tracked (`git checkout -- .`) → ПОВТОРНЫЙ status (честный `residual_modified` — case-collision/smudge revert'ом не лечатся; «стало чисто» не утверждается) → событие `kind:'dirty-after-prepare'` (reverted/untracked); untracked не трогаются. Политика revert+событие (не fail) утверждена — прогоны не блокируются, дефект виден в каждом таймлайне.
- **Wrapper говорит правду об окружении**: repoLine получает `[bootstrap already ran: `cmd` — dependencies are installed, do not re-install]`, `verificationSection` при выполненном bootstrap'е добавляет правило «Dependencies are ALREADY INSTALLED … go straight to the checks»; без bootstrap — байт-в-байт как раньше.
- **Санитария смежного**: admin-MCP `set_env` пересобирал repo-объекты из перечисленных полей и молча ДРОПНУЛ БЫ `bootstrap_command` при каждом изменении env — починено (`repoFields` с passthrough).
- **Источник грязи st3_agentic разобран** (таймбокс-репро): на коммитах середины июля оба файла были некононичны для prettier, а `npm run format` = `prettier --write src/**` и `npm run lint` — с вшитым `--fix`: любой агент, гонявший format/lint, переписывал чужие файлы. Текущий main полностью каноничен (дефект самоизлечился), тесты/линт ничего не мутируют — класс дефекта закрывает dirty-guard; рекомендация в st3_agentic — CI-гейт `prettier --check` (вынесено отдельной задачей).
- **Тесты** (все зелёные): unit — `bootstrap.spec.ts` (10, реальные sh/git: group-kill добивает backgrounded-потомка, rolling tail на ~200KB вывода, abort mid-flight, staged-остаток честно репортится), executor (6 новых: env/кэш/событие/wrapper, fail-fast без спауна, hang-breaker, abort→timeout, guard без bootstrap, чистое дерево без события; мок `./bootstrap` добавлен во все describe — guard теперь безусловный), config (`normalizeBootstrapTimeoutMs` + `pmCacheRoot`), wrapper (4). Интеграция — новый `claude-cli-bootstrap.spec.ts` (3: полный цикл с revert'ом и пробой `npm_config_cache` через реальный воркер, `exit 7` → failed с диагностикой, без bootstrap — ноль событий).
